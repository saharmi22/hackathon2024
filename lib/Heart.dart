...
class _HeartRateHRVScreenState extends State<HeartRateHRVScreen> {
  // Initial sample data
  List<double> heartRateData = [70, 72, 75, 73, 71, 70, 72, 74, 76, 75];
  List<double> hrvData = [50, 52, 53, 51, 49, 48, 50, 52, 53, 51];
  ...
  void _startTimer() {
    _timer = Timer.periodic(Duration(seconds: 5), (timer) {
      setState(() {
        if (_currentIndex < heartRateData.length - 1) {
          _currentIndex++;
        } else {
          _currentIndex = 0;
        }
  ...
  @override
  Widget build(BuildContext context) {
    double currentHeartRate = heartRateData[_currentIndex];
    double currentHRV = hrvData[_currentIndex];

    double averageHeartRate = calculateAverage(heartRateData);
    double averageHRV = calculateHRV(hrvData);
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 10),
                      Text(
                        '${currentHRV.toStringAsFixed(1)} ms',
                        style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.black87),
                      ),
                    ],
                  ),
            ...
            Container(
              ...
              child: LineChart(
                LineChartData(
                  ...
                  lineBarsData: [
                    LineChartBarData(
                      spots: List.generate(heartRateData.length, (index) => FlSpot(index.toDouble(), heartRateData[index])),
                      isCurved: true,
                      colors: [Colors.blue],
                      barWidth: 4,
                    ...
                    LineChartBarData(
                      spots: List.generate(hrvData.length, (index) => FlSpot(index.toDouble(), hrvData[index])),
                      isCurved: true,
                      colors: [Colors.green],
                      barWidth: 4,
                      isStrokeCapRound: true,
